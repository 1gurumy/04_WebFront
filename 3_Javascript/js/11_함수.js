...
btn1.addEventListener("click", function() {
  ...
  let result; // 입력된 값이 0인지 양수인지 음수인지 판별해서 저장
  ...
  // 중요!!! 전달되는 전달 인자의 값 순서가 중요하다.
  print1(result, value);

});

// 매개변수로 배열 전달하기
function arrayTest(arr, num) {  // arr, x
...
const twoNumberPlus = (otherFn) => {
  const input1 = Number(prompt("첫 번쨰 값"))
  const input2 = Number(prompt("두 번쨰 값"))
